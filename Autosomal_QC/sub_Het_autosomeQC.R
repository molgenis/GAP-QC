...
hetdata$outlier <- ifelse(hetdata$OHET - (coef(M)[1] + coef(M)[2]*hetdata$KB) 
                          < -4*sd(abs(M$residuals))|
                            hetdata$OHET-mean(hetdata$OHET)>4*sd(hetdata$OHET),
                          "Excluded" ,"Included")
...
ROH.het<-ggplot(hetdata, aes(x=KB,y=OHET))+
  ...
  geom_abline(slope = M$coefficients[2], 
              intercept = M$coefficients[1]-4*sd(abs(M$residuals)),colour="red",lwd=1.00)+
  scale_color_manual(values  = c("blue","black"))+
  guides(colour = guide_legend(override.aes = list(alpha=1)))+
  xlab("Total length of ROH (kb)")+ylab("Heterozygosity")+
  geom_hline(yintercept=mean(hetdata$OHET)+4*sd(hetdata$OHET),color="red",lwd=1.00)+
